...
public class MyQueryRetrievalModel {

    protected MyIndexReader indexReader;

    private HashMap<String, Long> myCollectionFreq = new HashMap<>();

    private HashMap<Integer, HashMap<String, Integer>> postingLists = new HashMap<>();

    public MyQueryRetrievalModel(MyIndexReader ixreader) {
    ...
    public List<Document> retrieveQuery( Query aQuery, int TopN ) throws IOException {
        ...
        for (String word: qWords) {
            ...
            if (collectionFreq != 0) {
                ...
                for (int[] p: postingList) {
                    int docid = p[0];
                    int freq = p[1];
                    HashMap<String, Integer> termToFreq = postingLists.getOrDefault(docid, new HashMap<>());
                    if (termToFreq.isEmpty()) {
                        postingLists.putIfAbsent(docid, termToFreq);
                    }
        ...
        // calculate the score
        ArrayList<Document> results = new ArrayList<>(postingLists.size());
        double totalTermFreq = indexReader.getTotalTermFreq();
        for (Integer docid: postingLists.keySet()) {
            ...
            double score = 1.0;
            double docLen = indexReader.docLength(docid);
    ...
    public HashMap<Integer, HashMap<String, Integer>> getPostingLists() {
        return postingLists;
    }

    public HashMap<String, Long> getMyCollectionFreq() {
        return myCollectionFreq;
    }
}
